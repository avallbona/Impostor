...
@pytest.fixture(autouse=True)
def real_admin():
    real_admin = get_user_model().objects.create(username=admin_username, password=admin_pass)
...
@pytest.fixture(autouse=True)
def real_user():
    real_user = get_user_model().objects.create(username=user_username, email=user_email, password=user_pass)
...
def real_user_with_supergroup(custom_settings):
    user, __ = get_user_model().objects.get_or_create(
        username='user-with-supergroup', email='user.with.supergroup@email.com', password='1234qwer'
    )
    user.set_password(user_with_supergroup_pass)
    user.save()

    from django.contrib.auth.models import Group
    impostor_group, __ = Group.objects.get_or_create(name=custom_settings.IMPOSTOR_GROUP)
